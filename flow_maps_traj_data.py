...
#     seg = (max(x_values) - min(x_values))/12   # Add edges to the graph with weights
#     for i in range(len(nodes)):
#         for j in range(len(nodes)):
#             weight = edges[i][j]
#             node1 = nodes[i]
...
#             # p1: nr leaving edges, p2: nr incoming edges
...
#     # Customize node shapes and sizes
#     node_patches = [patches.Circle((0, 0), max(out_edge_weights[node], in_edge_weights[node])/30, edgecolor='black', facecolor='black') for node in nodes]
...
# # Get trajectories
# t = Trajectory(r"algorithm_repository\Flow-Maps\airport_trajectories.txt")
# pos = read_from_json(r"algorithm_repository\Flow-Maps\airport_coords.json")
# # Extract x-values from the points
# x_values = [point[0] for point in pos.values()]
...
# # Normalize
# norm_factor_width=15
# norm_factor_height = 15
# negative_width=True
# negative_height=False
# scaling_width = -1 if negative_width else 0
# scaling_height = -1 if negative_height else 0
# x_values_norm = min_max_scaling(x_values, scaling_width*norm_factor_width, norm_factor_width)
# y_values_norm = min_max_scaling(y_values, scaling_height*norm_factor_height, norm_factor_height)
# pos_norm = dict()
# for i in range(len(x_values)):
#     pos_norm[getDictKeys(pos)[i]] = (x_values_norm[i], y_values_norm[i])
# pos = pos_norm
# nodes = t.unique_points
# trajectories = t.filterOnEndPoints(['SVO'])
# edges = t.constructWeightMatrix(trajectories)

# # Normalize edges
# min_edge, max_edge = np.min(edges), np.max(edges)
...
def draw_weighted_graph(nodes, edges, positions, title, save_path=""):
    # Create a graph
    G = nx.DiGraph()
    ...
    # ax.set_extent([min(x_values) - 1, max(x_values) + 1, min(y_values) - 1, max(y_values) + 1], crs=ccrs.PlateCarree())
    ...
    while edges_placed < len(G.edges()):
        ...
        for edge in G.edges():
            ...
            if G.edges[edge]['p1'] == p1_counters[node1] and G.edges[edge]['p2'] == p2_counters[node2]:
                ...
                # Map the x-coordinate of the starting point to a color value
                color_value = (y1 - min(pos[node][1] for node in nodes)) / (max(pos[node][1] for node in nodes) - min(pos[node][1] for node in nodes))
                # color_value = (x1 - min(pos[node][0] for node in nodes)) / (max(pos[node][0] for node in nodes) - min(pos[node][0] for node in nodes))
    ...
    # Customize node shapes and sizes
    node_patches = [patches.Circle((0, 0), max(out_edge_weights[node], in_edge_weights[node])/4, edgecolor='DarkRed', 
                                   facecolor='red') for node in nodes]
...
# pos = pos_norm
nodes = t.unique_points
trajectories = t.filterOnStartPoints(['SEA', 'LAX'])
edges = t.constructWeightMatrix(trajectories=None)
max_value = np.max(edges)
edges = t.constructWeightMatrix(trajectories=trajectories)
