...
def draw_weighted_graph(nodes, edges, positions):
    ...
    seg = (max(x_values) - min(x_values))/30   # Add edges to the graph with weights
    for i in range(len(nodes)):
        for j in range(len(nodes)):
            weight = edges[i][j]
            node1 = nodes[i]
...
norm_factor=7.5
negative=True
scaling = -1 if negative else 0
print(scaling)
x_values_norm = min_max_scaling(x_values, scaling*norm_factor, norm_factor)
y_values_norm = min_max_scaling(y_values, scaling*norm_factor, norm_factor)
pos_norm = dict()
for i in range(len(x_values)):
    pos_norm[getDictKeys(pos)[i]] = (x_values_norm[i], y_values_norm[i])
pos = pos_norm
nodes = t.unique_points
trajectories = t.filterOnEndPoints(['LHR', 'SVO'])
edges = t.constructWeightMatrix(trajectories)
...
mult = 2 if negative else 1
edges = (edges / np.max(edges)) * mult*norm_factor
